...
We can take a look at the information in different ways. For example, look at the first 10 rows out of `r format(initial_n, big.mark = ",")`.
...
Remember, the original dataset contains `r format(initial_n, big.mark = ",")` rows. After removing `r nrow(high_income_index)` observations, we are left with `r format(clean_n, big.mark = ",")` rows. Let's take a look at the result.
...
We could also explore some gender-related summaries.
