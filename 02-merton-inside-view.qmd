...
convergence_pd_data <- do.call(
  ...
  lapply(names(VT_convergence), function(sample_label) {
    ...
    histogram_counts <- hist(
      values,
      breaks = seq(4, 30, by = 0.25),
      plot = FALSE
    )$counts
    data.frame(
      sample_label = sample_label,
      x = 17.6,
      y = max(histogram_counts) * 0.95,
      label = paste0(
...
ggplot2::ggplot(
  ...
  ggplot2::facet_wrap(~sample_label, ncol = 3, scales = "free_y") +
...
Figure @fig-vt-convergence-hists makes the sampling error visible. With 100 simulated terminal values, the simulated probability can be noticeably away from the analytical Merton value. With 1,000 and then 10,000 simulated terminal values, the red area becomes a more stable approximation of the analytical risk-neutral probability $N(-d_2)$.

Here are 100,000 simulated values of $V_T$ shown in a histogram. The reference for this section is section 15.1 in @Hull. This repeats the same terminal-value simulation with a much larger sample.
