...
Instead of drawing complete paths, we can also simulate terminal asset values directly from the risk-neutral distribution of $V_T$. The resulting distribution is log-normal. This is useful because the default event depends only on whether the terminal value is below $D$.
...
This mapping is useful because it turns the credit problem into a contingent-claim problem. Default is the out-of-the-money region of equity. Survival is the in-the-money region. The debt payment $D$ becomes the default boundary. The option formula in section 3.2 is therefore not an unrelated Black-Scholes detour; it is the pricing formula for the equity claim once equity is understood as a call on the firm's assets. Observed equity value and equity volatility can then be used to infer the unobserved asset value and asset volatility, and those asset quantities determine the model-implied probability of default.

It also clarifies what is being valued. The call formula values the equity claim, not the whole firm directly. In the Hull/Merton example, section 3.2 inferred a market asset value of $V_0=`r fmt_num(V0, 4)`$. The observed equity value is $E_0=`r fmt_num(E0, 0)`$, so the model-implied market value of debt is $B_0=V_0-E_0=`r fmt_num(market_value_debt, 4)`$. A buyer of the whole firm as a package of market-value assets would look at $V_0$. A buyer of only the shares would be buying the junior residual claim worth $E_0$. The difference between those two values is the debt claim, not a contradiction in valuation.
...
ggplot2::ggplot(
  ...
  ggplot2::geom_line(linewidth = 1.25, color = "black") +
  ggplot2::geom_vline(xintercept = D, linetype = "dashed", linewidth = 0.7) +
  ggplot2::annotate(
    "label",
    x = 6.1,
  ...
  ggplot2::coord_cartesian(xlim = c(5, 20), ylim = c(0, 10), expand = FALSE) +
  ggplot2::labs(
