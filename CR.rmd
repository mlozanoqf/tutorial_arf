...
```{r}
# Add a sex variable. sex = 1 female; sex = 0 male.
set.seed(1)
...
There are some guys with a very high income. We should explore further and investigate if this is a valid observation or simply a mistake in the database.
...
One guy is not only rich, he is 144 years old. So, my decision is to drop these 9 observations as I do not think these are valid records. Remember the database has originally 29,092 rows. Here I drop the observations:
...
We have 29,074 observations in \texttt{loan\_data}. The code above randomly selects $29074 \times (2/3)=19382$ rows to form the \texttt{training\_set}. The \texttt{test\_set} are the remaining $29074-19382=9692$ rows. The random selection is highly recommended as the \texttt{loan\_data} may have some structure or sorting that could bias our model estimation and negatively impact our model test. 
...
Apparently, there is a negative relationship between age and loan status. The AIC value (13580) is useful when comparing models. We are interested in models with the lowest AIC because this is equivalent as a higher goodness of fit. Let's estimate another simple model where the interest rate category is used as a predictor of the \texttt{loan\_status}.
...
The AIC is a lower (12000), so we have a better model now. 
...
In \texttt{log\_model\_multi}, the AIC value is the lowest so far (11875), so this should be considered as the best model at the moment. The \texttt{summary} function shows the significance levels of the estimators, but we are currently more interested in the goodness of fit of the models because we are interested in conducting predictions about the \texttt{loan\_status}. This is, we are interested to use a model to find out whether new applicants are expected to default or not, rather than in the credit risk factors. 
...
Let's take \texttt{log\_model\_age} and \texttt{log\_model\_ir} from the previous subsection to carry out a simple prediction exercise. We start this prediction exercise by identifying one observation in the test set and ask the models to estimate the \texttt{loan\_status}. 
...
log_model_age_pred <- predict(log_model_age, newdata = test_case, 
                              type = "response")
# Rememer the test_case is only one observation (one row).
log_model_ir_pred <- predict(log_model_ir, newdata = test_case, 
                              type = "response")
log_model_multi_pred <- predict(log_model_multi, newdata = test_case, 
                              type = "response")
data.frame(cbind(log_model_age_pred, log_model_ir_pred, log_model_multi_pred,
                 test_case[1]))
```

These values are low as they are close to 0. We could interpret this as a certain ability of the \texttt{log\_model\_age} and \texttt{log\_model\_ir} models to predict this single case from the test set. However, several questions remains unanswered and requires further analysis. 
...
log_model_ir_predall <- range(pred_log_model_ir, na.rm = TRUE)
log_model_multi_predall <- range(pred_log_model_multi, na.rm = TRUE)


data.frame(rbind(log_model_age_predall, log_model_ir_predall,
                 log_model_multi_predall))

```

The \texttt{log\_model_age} has loan status from 0.08417892 to 0.1165453 and \texttt{log\_model\_ir} from 0.05049310 to 0.3981023. The maximum values are far from 1, so apparently these two simple models are not very good at identifying defaults. Let's see more details about these ranges in a plot.

Let's explore the \texttt{log\_model_age}:

```{r}
library(tidyr)
pred_age_ir <- as.data.frame(cbind(pred_log_model_ir,
                                   pred_log_model_multi))
pred_age_ir <- gather(pred_age_ir, prediction)
ggplot(pred_age_ir, aes(x=value, fill = prediction)) + 
  geom_density(alpha=0.4)
#plot(pred_log_model_age)
```


```{r}
pred <- as.data.frame(cbind(pred_log_model_age, test_set$loan_amnt))
ggplot(pred, aes(V2, pred_log_model_age)) +
geom_point()
...
range(predictions_all_full, na.rm = TRUE)
```
...
```{r}
full_age <- data.frame(predictions_all_full, pred_log_model_age)
boxplot(full_age)
```
...
```{r}
# Basic box plot
ggplot(full_age, aes(x=predictions_all_full, x=pred_log_model_age)) +
geom_boxplot()
