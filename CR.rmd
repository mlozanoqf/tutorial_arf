...
This section heavily relies on the DataCamp course *Credit Risk Modeling in R* by Lore Dirick. 


## Explore the database.

We have a loan database called \texttt{loan\_data\_ch1.rds}. Let's load the data and then understand its structure. You can download these data files from the DataCamp course.
...
This could be a typical database taken from a financial institution like a bank or a firm that uses credit to sell their products or services. Here, we have 29,092 observations of 8 variables. Each observation corresponds to one individual loan and each variable allow us to understand the individual and the loan characteristics. One important variable is \texttt{loan\_status} the value of 0 is no default and the value of 1 is default. A default occurs when a borrower is unable to make timely payments, misses payments, or avoids or stops making payments on interest or principal owed. Then, the definition of default depends on the analysis.
...
We can look at the information in different ways. For example, look at the first 10 rows (out of 29,092) and their corresponding 8 variables.
...
These tables illustrate the data structure and contents. We can always store the function result in one variable. This can be useful to extract specific values of this \texttt{CrossTable} function. Look how we can do this.

```{r}
# Store results in t1
t1 <- CrossTable(loan_data$home_ownership)
```
Now, \texttt{t1} has the table information. Let's look at the row that stores the percentage proportion.

```{r}
t1$prop.row
```

We can even store an individual value for RENT into a new variable. 

```{r}
prop.rent.t1 <- t1$prop.row[1, "RENT"]
# Show the value of the prop.rent.t1 variable
prop.rent.t1
```

We can also use two variables instead of one. Instead of counting for home ownership we can add a second dimension like \texttt{loan\_status}. This allows us to create more informative tables. 
...
```{r}
hist(loan_data$int_rate, 50, main = "Histogram of interest rate", 
     xlab = "Interest rate")
```
...
```{r}
# As a rule of thumb, we set the number of breaks as:
...
There is one guy with a very high income. We should explore further and investigate if this is a valid observation or simply a mistake in the database.

```{r}
# Extract the row of this high income individual.
t(loan_data[which.max(loan_data$annual_inc),])
```

This guy is not only rich, he is 144 years old. So, my decision is to drop this observation number 19,486 as I do not think this is a valid record. Remember the database has originally 29,092 rows, this guy is located in the 19,486 row. Here I drop the observation:

```{r}
loan_data <- loan_data[-19486,]
plot(loan_data$annual_inc, ylab = "Annual Income")
```
There are still some high values. This is the new histogram with a truncated x-axis.
...
Starting from here, we will use a slightly different database called \texttt{loan\_data\_ch2.rds}. This is simply because our example relies on Lore Dirick credit risk DataCamp course.

```{r}
# Load a slightly different database.
loan_data2 <- readRDS("loan_data_ch2.rds")
# It is convenient to set the loan_status as factor.
loan_data2$loan_status <- as.factor(loan_data2$loan_status)
# The sample function will always lead to the same result by setting a seed.
set.seed(567)
index_train <- cbind(runif(1 : nrow(loan_data2), 0 , 1), 
                     c(1 : nrow(loan_data2)))
index_train <- order(index_train[, 1])
index_train <- index_train[1: (2/3 * nrow(loan_data2))]
# Originally we used "sample" but it has difficulties at replicating.
# index_train <- sample(1:nrow(loan_data2), 2/3 * nrow(loan_data2))
# Create training set: training_set
training_set <- loan_data2[index_train, ]
# Create test set: test_set
test_set <- loan_data2[-index_train, ]
```



We have 29,091 observations in \texttt{loan\_data2}. The code above randomly selects $29091 \times (2/3)=19394$ rows to form the \texttt{training\_set}. The \texttt{test\_set} are the remaining $29091-19394=9697$ rows. The random selection is highly recommended as the \texttt{loan\_data2} may have some structure or sorting that could bias our model estimation and negatively impact our model test. 
...
Variables as factors are useful for model estimation. Factors allow the model to understand that these are not continuous variables, but categorical variables that can have only a limited amount of values. 
...
log_model <- glm(loan_status ~ age , family = "binomial", 
                 data = training_set)
log_model
```

Apparently, there is a negative relationship between age and loan status. The AIC value (13750) is useful when comparing models. We are interested in models with the lowest AIC because this is equivalent as a higher goodness of fit. Let's estimate another simple model where the interest rate category is used as a predictor of the \texttt{loan\_status}.
...
The AIC is a bit higher (13380), so we have a worst model now. Here, we show the interest rate categories.
...
Using one predictor could be a limited approach. Let's add some more. Also, let's introduce the \texttt{summary} function to extract more information about the model estimation. The \texttt{log\_model\_multi} below assumes that the loan status depend on the age, interest rate category, grade, loan amount, and annual income. 

```{r}
# Multiple variables in a logistic regression model.
log_model_multi <- glm(loan_status ~ age + ir_cat + grade + loan_amnt + 
                         annual_inc , family = "binomial", 
                       data = training_set)
# Obtain significance levels using summary().
summary(log_model_multi)
```

In \texttt{log\_model\_multi}, the AIC value is the lowest so far (13236), so this should be considered as the best model at the moment. The \texttt{summary} function shows the significance levels of the estimators, but we are currently more interested in the goodness of fit of the models because we are interested in conducting predictions about the \texttt{loan\_status}. This is, we are interested to use a model to find out whether new applicants are expected to default or not, rather than in the credit risk factors. 
...
Let's take \texttt{log\_model} and \texttt{log\_model\_cat} from the previous subsection to carry out a simple prediction exercise. We start this prediction exercise by identifying one observation in the test set and ask the models to estimate the \texttt{loan\_status}. 
...
We know in advance that the \texttt{loan\_status} of this observation taken from the test set is 0. In other words, a good credit risk model should estimate a no default for this new data.
...
```{r}
# Estimate the loan status with log_model and log_model_cat models.
log_model_pred <- predict(log_model, newdata = test_case, type = "response")
# Rememer the test_case is only one observation (one row).
log_model_cat_pred <- predict(log_model_cat, newdata = test_case, 
                              type = "response")
data.frame(cbind(log_model_pred, log_model_cat_pred, test_case[1]))
```

These values are low as they are close to 0. We could interpret this as a certain ability of the \texttt{log\_model} and \texttt{log\_model\_cat} models to predict this single case from the test set. However, several questions remains unanswered and requires further analysis. 

For example: What about the rest of the cases in the test set? We have 9,697 observations in the test set and in the example above we only test for the first one. Fortunately, this issue is easy to address as we only need to change the \texttt{newdata} parameter in the \texttt{predict} function. In particular, instead of \texttt{newdata = test\_case} we can change it to \texttt{newdata = test\_set}.

```{r}
# Estimate the loan status with log_model and log_model_cat models.
pred_log_model <- predict(log_model, newdata = test_set, type = "response")
# Now newdata = test_set so we are testing all the test set observations.
pred_log_model_cat <- predict(log_model_cat, newdata = test_set, 
                              type = "response")
# The range of the estimated loan status.
log_model_predall <- range(pred_log_model)
log_model_cat_predall <- range(pred_log_model_cat)

data.frame(rbind(log_model_predall, log_model_cat_predall))

```

We have 9,697 estimated loan status per model. The \texttt{log\_model} has loan status from 0.06040744 to 0.12095957 and \texttt{log\_model\_cat} from 0.0538592 to 0.1792169. The maximum values are far from 1, so apparently these two simple models are not very good at identifying defaults. Let's see more details about these ranges in a plot.

Let's explore the \texttt{log\_model}:

```{r}
plot(pred_log_model)
```
The \texttt{log\_model} fails to predict values ranging from 0 to 1. In fact, these values are quite concentrated in a very small range of values. As a consequence, this model fails to differentiate between default and no default predictions. A boxplot reveals the same failure.

```{r}
boxplot(pred_log_model)
```
This does not look very good as the maximum values for loan status are concentrated around 0.12. It's time to use a better model. Presumably, a model which considers all available predictors could be better for predicting the \texttt{loan\_status}. Remember we estimate models using the training set data and then we use the test set to conduct the predictions. Evaluating a model performance with the same data as we use to estimate the model is problematic. 

```{r}
# Logistic regression model using all available predictors in the data set.
log_model_full <- glm(loan_status ~ ., family = "binomial", 
                      data = training_set)
...
```{r}
plot(predictions_all_full, col = ifelse(predictions_all_full < 0.15,
                                        "red","green"), pch = 16, cex = 0.5)
```
Green represent no default and red default. Let's set up the rule to convert the estimated loan status between 0 and 0.5 into a binary variable 0 or 1. 
...
Note that the rows numbers in the table above are 1, 2, 18, 26, 27 and 28. These are not 1, 2, 3, 4, 5 and 6 because the \texttt{test\_set} has 9,697 rows selected randomly out of the 29,091 observations in \texttt{loan\_data2}. So, the row numbers in the table above correspond to the original place in \texttt{loan\_data2}.
...
strategy_pred_log_model <- strategy_bank(as.numeric(pred_log_model))
...
plot(strategy_pred_log_model$accept_rate, 
     ...
     ylab = "Bad rate", lwd = 2, main = "log_model")
...
ROC_log_model <- roc(test_set$loan_status, pred_log_model)
...
```{r}
set.seed(2020)
pred_rand_model <- runif(length(pred_log_model))
...
These simulations can be interpreted as the evolution of the value of the firm's assets in 10 parallel universes. In the past, some students have called this figure *la gráfica de los pelitos*, this is OK as long as you understand the figure implications. The right name is 10 simulated geometric brownian processes. Let's count the cases in which $V_t<D$ note that here I use $t$ and not $T$, so we are counting the cases in which at least at some time $t$ the value of the assets were lower than 10. 
...
pd.E # vemos qué hay en pd.E
# Busco el valor de pd más parecido a 0.05
index <- which(almost.equal(pd.E, 0.05, tolerance = 0.01))
# ¿Qué número de observación es? (da 38 con los datos del tutorial)
index
# ¿Qué valor de equity me da una pd de 0.05?
