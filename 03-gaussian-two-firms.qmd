...
The matrix <tt>`x2`</tt> length is `r format(n, big.mark = ",")` for each firm. In other words, we have `r format(n, big.mark = ",")` observations of the performance measure or $z$-score for two firms that are related. This matrix is big, but we can visualize the header (the first six observations).

```{r}
kable(head(x2), caption = "Firms' performance x2.", row.names = TRUE)
```

Remember the cumulative probabilities of default are `r paste(fmt_pct(pd, 0), collapse = ", ")` for the maturities of 1, 2, 3, 4 and 5 years respectively. How do we extract those cases in which both firms will default in 5 years? We first identify the simulated rows in which both firms default at the same time. Note that in all such cases the $x$ values are indeed below `r fmt_num(x.y[5], 6)`.
...
kable(head(both), caption = "Cases in which both firms default in 5 years.", 
      row.names = TRUE)
```
In total, we have `r nrow(both)` cases in which both firms default at the same time. The first simulated row numbers are `r paste(head(rownames(both), 3), collapse = ", ")`, and so on. It is easy to know the total cases if we count the number of rows.

How do we extract those cases in which any firm will default in 5 years? This is, only firm 1, only firm 2, or both at the same time. This is a less strict condition so we would expect to have more cases to match this new criteria compared with <tt>`both`</tt>. Note that in all cases at least one firm is indeed below `r fmt_num(x.y[5], 6)`. The table below shows the first simulated rows that satisfy this condition.
...
Now we have `r format(nrow(any), big.mark = ",")` cases. Considerably more as the <tt>`|`</tt> restriction is less strict than the <tt>`&`</tt>. Let's see the cases in which only firm 1 defaults.
...
firm1.pd <- onlyf1.pd + both.pd
firm2.pd <- onlyf2.pd + both.pd
one_from_only <- onlyf1.pd[5] + onlyf2.pd[5]
one_from_any_both <- any.pd[5] - both.pd[5]
...
Note that `r fmt_pct(pd[5], 0)` is the theoretical probability that one firm will default in 5 years. Here, this `r fmt_pct(pd[5], 0)` is `r fmt_pct(firm1.pd[5], 2)` for firm 1 and `r fmt_pct(firm2.pd[5], 2)` for firm 2 when the data is simulated. 

We can perform a simple test to verify that everything is alright. For example, this equation must hold: <tt>`onlyf1+onlyf2=one`</tt>. Substituting for the year 5: $`r fmt_num(onlyf1.pd[5], 4)`+`r fmt_num(onlyf2.pd[5], 4)`=`r fmt_num(one_from_only, 4)`$. As you can see, everything is alright. This equation must hold as well: <tt>`any-both=one`</tt>. Substituting for the year 5: $`r fmt_num(any.pd[5], 4)`-`r fmt_num(both.pd[5], 4)`=`r fmt_num(one_from_any_both, 4)`$.

Let's visualize all `r format(nrow(x2), big.mark = ",")` cases. Each dot represents a couple of Firm 1 and Firm 2 $x$ values and the dotted lines the threshold that represents the probability of default in 5 years.

```{r}
#| label: fig-a1sc
#| fig-cap: "All simulated cases."
...
These `r format(nrow(x2), big.mark = ",")` observations are highly concentrated around the mean which is very close to zero. This can be also easily seen in the following density plot.


```{r}
#| label: fig-a1scadv
#| fig-cap: "All simulated cases: A density view."
...
The values within the plot represent the number of cases. Here, we have `r nrow(X[["both", 5]])` times (out of `r format(n, big.mark = ",")`) in which both firms default at the same time in 5 years. Note that this is a cumulative probability of default.
