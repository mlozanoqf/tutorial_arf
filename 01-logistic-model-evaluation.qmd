...
The second rule has higher accuracy, but it detects no defaults at all. This is unacceptable for credit screening because the bank would accept all defaulting applicants. The full model with a cutoff of 0.15 has lower overall accuracy, but it detects 752 defaults and reduces the number of defaulting loans accepted by the bank. In imbalanced classification problems, accuracy alone can reward the wrong behavior.
...
Now the cutoff is 0.1994621. This splits the predicted default probabilities into two parts: higher than the cutoff is a default prediction, and lower than the cutoff is a no-default prediction. Here are the cutoff values as a function of quantiles.
...
We can also look at the detail of the 0.1994621 cutoff by comparing two columns: the one on the left with the actual loan status, and the one on the right with the binary prediction created from the estimated default probability.
