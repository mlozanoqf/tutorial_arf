...
For comparison, the same idea can be drawn with `ggplot2`. The original plot remains above; the next plot is only a candidate replacement so we can compare the visual treatment of the red and green areas.

```{r}
#| label: trartpod-ggplot-candidate
...
ggplot2::ggplot(normal_density_data, ggplot2::aes(x = x, y = density)) +
  ...
  ggplot2::labs(
    x = "d values (-d_2 is the dotted line)",
    y = "Density"
  ) +
...
Taken together, @fig-hwftvotaactvoaatz and @fig-hwftvotaatzactvota show that the numerical solution is locally coherent in each parameter direction. The next figures extend the same idea to the joint surface, where $V_0$ and $\sigma_V$ move at the same time.

We can show the same objective function as a 3D plot.
...
The following plot is useful in HTML because it can be shown as an interactive surface. The previous `persp()` plot is static; `plotly` gives a browser-based version that can be rotated.
