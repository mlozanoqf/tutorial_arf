...
threshold_six_months <- -2.5
threshold_one_year <- -2
pd_six_months <- pnorm(threshold_six_months)
pd_one_year <- pnorm(threshold_one_year)
density_extreme_4 <- dnorm(4)
```
...
Assume the firm will default in six months as long as the firm performance $x$ in six months is lower than a given threshold: $\bar{x}=`r threshold_six_months`$. The threshold $\bar{x}=`r threshold_six_months`$ in the Gaussian copula model is equivalent to the $-d_2$ in the Merton model. Then, the six months firm's probability of default in math notation is $N(`r threshold_six_months`)$, or <tt>pnorm(`r threshold_six_months`) = `r fmt_num(pd_six_months, 9)`</tt> in R code, which is very low as it is close to 0. It is the red area below, which is `r fmt_pct(pd_six_months, 2)` of the 100% possible area under the curve.
...
plot(x, D, type = "l", lwd = 3, col = "blue" , 
     ylab = "Standard normal density function: dnorm(x)")
abline(h = 0, lty = 2)
abline(v = threshold_six_months, lty = 2)
polygon(c(x[x < threshold_six_months], threshold_six_months),
        c(D[x < threshold_six_months], 0), col = "red")
```

Now assume the firm will default in one year as long as the firm performance $x$ in one year is lower than a higher given threshold: $\bar{x}=`r threshold_one_year`$. Then, the one year probability of default is $N(`r threshold_one_year`)$ or <tt>pnorm(`r threshold_one_year`) = `r fmt_num(pd_one_year, 9)`</tt> which is still low but higher than the six months probability of default. Note that the new red area is `r fmt_pct(pd_one_year, 2)`.
...
plot(x, D, type = "l", lwd = 3, col = "blue" , 
     ylab = "Standard normal density function: dnorm(x)")
abline(h = 0, lty = 2)
abline(v = threshold_one_year, lty = 2)
polygon(c(x[x < threshold_one_year], threshold_one_year),
        c(D[x < threshold_one_year], 0), col = "red")
```

At the moment, we have not discussed how to estimate the threshold values $\bar{x}$. However, we can say that the Gaussian copula model assumes that the probability of default increases as time passes. As if the firm will eventually default in the future. The six month probability of default `r fmt_pct(pd_six_months, 2)` is low because it is not very likely that the future firm performance lies below `r threshold_six_months` in six months. The one year probability of default `r fmt_pct(pd_one_year, 2)` is a bit higher because it is more likely that the future firm performance lies below `r threshold_one_year` in one year.
...
```{r}
pnorm(threshold_six_months)
pnorm(threshold_one_year)
```
