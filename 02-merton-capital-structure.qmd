...
Which action produces the lower model-implied probability of default? Should she reduce the promised debt payment $D$ by 2, or increase the equity value $E_0$ by 2 through a capital injection?
...
```{r capital-structure-scenario-summary}
capital_structure_summary <- data.frame(
  ...
  `Initial position` = c(
    paste0("$E_0 = ", fmt_num(E0, 0), "$, $D = ", fmt_num(D, 0), "$, $T = ", fmt_num(TT, 0), "$"),
    paste0("$E_0 = ", fmt_num(scenario2_E0, 0), "$, $D = ", fmt_num(scenario2_D, 0), "$, $T = ", fmt_num(TT, 0), "$"),
    paste0("$E_0 = ", fmt_num(E0, 0), "$, $D = ", fmt_num(scenario3_D, 0), "$, $T = ", fmt_num(scenario3_T, 1), "$")
  ),
  `Initial risk-neutral PD` = c(
...
```{r}
#| label: fig-gppodacs
#| fig-cap: "Initial capital-structure position and Merton risk-neutral PD."
...
```{r}
#| label: fig-gspodacs
#| fig-cap: "Debt reduction versus equity injection from the initial position."
plot_capital_structure(
  curve_data = equity_curve_data,
  color_values = colors,
  x_label = paste0("Promised debt payment. Initially, D=", fmt_num(D, 0)),
...
Consider a different initial situation. Now, $E_0=2$ and $D=2$ while the remaining observed inputs are unchanged. In this case, the risk-neutral probability of default is `r fmt_pct(pd_scenario_2_initial, 2)`. Which action produces the lower model-implied probability of default? Should she reduce the promised debt payment $D$ by 1, or increase the equity value $E_0$ by 1?
...
```{r}
#| label: fig-gp2podacs
#| fig-cap: "Alternative starting balance sheet and Merton risk-neutral PD."
...
```{r}
#| label: fig-gs2podacs
#| fig-cap: "Debt reduction versus equity injection from the alternative balance sheet."
plot_capital_structure(
  curve_data = scenario2_curve_data,
  color_values = scenario2_colors,
  x_label = paste0(
...
```{r}
#| label: fig-gp3mdolm
#| fig-cap: "Initial maturity-debt position and Merton risk-neutral PD."
...
```{r}
#| label: fig-ga3mdolm
#| fig-cap: "Higher promised debt payment versus longer maturity."
plot_capital_structure(
  curve_data = maturity_curve_data,
  color_values = scenario3_colors,
  x_label = paste0(
