...
set.seed(1)
sex_1 <- rbinom(n = nrow(loan_data[loan_data$loan_st == 1, ]), 
                size = 1, prob = 0.3) # Let's make females default less.
sex_0 <- rbinom(n = nrow(loan_data[loan_data$loan_st == 0, ]), 
                size = 1, prob = 0.55) # Let's make females no default more.
sex <- ifelse(loan_data$loan_st == 1, sex_1, sex_0)
loan_data$sex <- sex
loan_data$sex <- as.factor(loan_data$sex)

# Add region variable.
set.seed(1)
row_def <- nrow(loan_data[loan_data$loan_st == 1, ])
row_nodef <- nrow(loan_data[loan_data$loan_st == 0, ])
...
region <- ifelse(loan_data$loan_st == 1, 
...
```{r}
colnames(loan_data)[1] <- "loan_st"
colnames(loan_data)[2] <- "l_amnt"
colnames(loan_data)[3] <- "int"
colnames(loan_data)[5] <- "emp_len"
colnames(loan_data)[6] <- "home"
colnames(loan_data)[7] <- "income"
colnames(loan_data)
```
...
Now, instead of looking the details of the first 10 rows, we can summarize with respect to <tt>`home`</tt> with the <tt>`CrossTable()`</tt> function.

```{r CrossTable as a way to summarize data}
CrossTable(loan_data$home)
```
...
CrossTable(loan_data$home, loan_data$loan_st, prop.r = TRUE,
...
high_income <- loan_data[(loan_data$income > 1000000), ]
high_income
```
...
loan_data$loan_st <- as.factor(loan_data$loan_st)
set.seed(567)
...
log_model_age <- glm(loan_st ~ age, family = "binomial", 
                     data = training_set)
log_model_age
```
...
log_model_ir <- glm(formula = loan_st ~ int, family = "binomial", 
                     data = training_set)
...
log_model_multi <- glm(loan_st ~ age + int + grade + log(l_amnt) + 
                         log(income) , family = "binomial", 
                       data = training_set)
# Obtain significance levels using summary().
summary(log_model_multi)
```

Our multi-factor model works well. In <tt>`log_model_multi`</tt>, the AIC value is the lowest so far (13050 versus 13220), so this should be considered as the best model at the moment. The <tt>`summary()`</tt> function shows the significance levels of the estimators, but we are currently more interested in the goodness of fit of the models because we want to conduct predictions about the <tt>`loan_st`</tt>. This is, we are interested to use a model to find out whether new applicants in the test set are expected to default or not, rather than in the applicants' credit risk factors. This is why we are concentrated in AIC now.
...
Let's take our three models: <tt>`log_model_age`</tt>, <tt>`log_model_ir`</tt> and <tt>`log_model_multi`</tt> from the previous subsection to carry out a simple prediction exercise. We start by identifying one observation in the test set and ask the models to estimate the <tt>`loan_st`</tt>. This is, we take the first guy age, then we apply the <tt>`log_model_age`</tt> model, and compare the predicted <tt>`loan_st`</tt> with respect to what really happened. Remember we know what really happened with this guy because we have the information in the test set. Every model is expected to produce different <tt>`loan_st`</tt> estimates. If the model is good, then the predicted <tt>`loan_st`</tt> will match what really happened.
...
Narrow ranges (the difference between higher and lower <tt>`loan_st`</tt> predicted values) could be problematic because the model could not be able to differentiate between defaults (predictions closer to 1) and no-defaults (predictions closer to 0). The higher AIC corresponds to the worst model and the lower AIC to the best model. Here, we can see some consistency because the best model according to the AIC, corresponds to the model with the higher prediction range.

Let's explore all the predicted <tt>`loan_st`</tt> values for the  <tt>`log_model_age`</tt>:
...
ggplot(pred_ir_multi, aes(x = value, fill = key)) + 
  ...
  theme(legend.position = "bottom", legend.title = element_blank())
```
Looks better, but not quite. Presumably, a model which considers all available predictors could be better for predicting the <tt>`loan_st`</tt>. 

```{r log_model_full estimation and prediction}
# Logistic regression model using all available predictors in the data set.
log_model_full <- glm(loan_st ~ age + int + grade + log(l_amnt) + 
                        log(income) + emp_len + home + sex +
                      region, family = "binomial", data = training_set)
#glm(loan_st ~ ., family = "binomial", data = training_set)
...
Now, the range is wider. A wider range means that the <tt>`loan_st`</tt> estimates are now closer to 1. This is good because we need the model to be able to predict both no-defaults (0) and defaults (1). Let's see a prediction comparisons of <tt>`log_model_multi`</tt> and <tt>`log_model_full`</tt>.
...
Here, predicted <tt>`loan_st`</tt> values at the left of the dashed line represent no default and values at the right of the dashed line represent default. Let's set up the rule to convert the estimated loan status into a binary variable 0 or 1. See how this transformation takes place:
...
(cbind(predictions_all_full, pred_cutoff_15, 
           as.data.frame.numeric(test_set$loan_st)))[101:110,]
```
...
table(test_set$loan_st, pred_cutoff_15)
```
...
CrossTable(test_set$loan_st, pred_cutoff_15, prop.r = TRUE,
           prop.c = FALSE, prop.t = FALSE, prop.chisq = FALSE)
```
...
CrossTable(test_set$loan_st, pred_full_20, prop.r = TRUE,
           prop.c = FALSE, prop.t = FALSE, prop.chisq = FALSE)
```
...
true_and_predval <- cbind.data.frame(test_set$loan_st, pred_full_20,
                                     test_set$loan_st==pred_full_20)
# Show some values.
...
We can evaluate our loan application rule as we have the corresponding real values in <tt>`loan_st`</tt>. 
...
head(data.frame(true_and_predval[,1:2], decision = 
                ...
                evaluation = ifelse(true_and_predval$pred_full_20 == 1, 
                                    ...
                                    ifelse(true_and_predval$pred_full_20 == 0 &
  true_and_predval$`test_set$loan_st` == 0,
...
true_and_predval <- cbind.data.frame(test_set$loan_st, pred_full_35)
...
for (i in 1:21){
  ...
  pred_as_good <- test_set$loan_st[pred_i == 0]
...
```{r}
ROC_all_full <- roc(test_set$loan_st, predictions_all_full)
ROC_log_model <- roc(test_set$loan_st, pred_log_model_age)
...
ROC_rand <- roc(test_set$loan_st, pred_rand_model)
# Draw the ROCs on one plot
...
```{r}
pred_perfect_model <- as.numeric(test_set$loan_st)
ROC_perfect <- roc(test_set$loan_st, pred_perfect_model)
