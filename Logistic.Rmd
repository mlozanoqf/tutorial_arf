...
```{r}
colnames(dat)[1] <- "loan_st"
colnames(dat)[2] <- "l_amnt"
colnames(dat)[3] <- "int"
colnames(dat)[5] <- "emp_len"
colnames(dat)[6] <- "home"
colnames(dat)[7] <- "income"
colnames(dat)
```
...
Note that sex is 1 for female and 0 for male. Now, instead of looking the details of the first 10 rows, we can summarize with respect to <tt>`home`</tt> with the <tt>`CrossTable()`</tt> function.
...
We have 29,083 observations in <tt>`dat`</tt>. The code above randomly selects $29083 \times (2/3)=19388$ rows to form the <tt>`train`</tt>. The <tt>`test`</tt> are the remaining $29083-19388=9695$ rows. The random selection is highly recommended as the <tt>`dat`</tt> may have some structure or sorting that could bias our model estimation and negatively impact our model test. For example, imagine that for some weird reason the database is sorted in such a way that the first observations are all no default. If that is the case, then the training and the test set would not have portions of default and no default cases and we may distort the whole analysis. The random selection allow us to replicate a real situation in which our database is unsorted, with different characteristics.

Let's see if the recent created <tt>`train`</tt> and <tt>`test`</tt> have the same basic properties than <tt>`dat`</tt>.

```{r}
dat_sum <- table(dat$loan_st)/sum(table(dat$loan_st))
train_sum <- table(train$loan_st)/sum(table(train$loan_st))
test_sum <- table(test$loan_st)/sum(table(test$loan_st))
rbind(dat_sum, train_sum, test_sum)
```
...
# Collect all.
predictions <- rbind("logi_age" = logi_age_pred, 
                     "logi_int" = logi_int_pred, 
                     "logi_multi" = logi_multi_pred)
# Prepare a table.
colnames(predictions) <- "Loan status predictions for John Doe."
predictions
```
...
predictions <- rbind("logi_age" = range(pred_logi_age), 
                     "logi_int" = range(pred_logi_int),
                     "logi_multi" = range(pred_logi_multi))
aic <- rbind(logi_age$aic, logi_int$aic, logi_multi$aic)
predictions <- cbind(predictions, aic)
colnames(predictions) <- c("min(loan_st)", "max(loan_st)", "AIC")
predictions
```
...
Let's visualize the predictions of the <tt>`logi_int`</tt> and <tt>`logi_multi`</tt>.

```{r}
pred_ir_multi <- data.frame(cbind(pred_logi_age, pred_logi_int,
                                  pred_logi_multi))
pred_ir_multi <- gather(pred_ir_multi, key = "model",  value = "pred")
```


```{r fig.cap = "Interest rate and multi models predictions histograms."}
ggplot(pred_ir_multi[pred_ir_multi$model != "pred_logi_age",], 
...
ggplot(pred_ir_multi, aes(x = pred, y = model, fill = model)) + 
  geom_boxplot() +
  labs(y = "Model",
       x = "Default prediction",
...
#glm(loan_st ~ ., family = "binomial", data = train)
                  
# Loan status predictions for all test set elements.
predictions_all_full <- predict(logi_full, newdata = test, type = "response")
...
caso.m <- test[m,]
caso.m
predm <- predict(logi_full, newdata = caso.m, type = "response")
predm
```

Now, the range is wider. A wider range means that the <tt>`loan_st`</tt> estimates are now closer to 1. This is good because we need the model to be able to predict both no-defaults (0) and defaults (1). Let's see a prediction comparisons of <tt>`logi_multi`</tt> and <tt>`logi_full`</tt>.

```{r fig.cap = "Multi and full models predictions histograms."}
predictions_multi_full <-as.data.frame(cbind(pred_logi_multi, 
...
pred_logi_full <- data.frame(predictions_all_full)
pred_logi_full <- gather(pred_logi_full)
pred_logi_full <- mutate(pred_logi_full, 
                              def = ifelse(value < 0.15, 0, 1))
pred_logi_full$def <- as.factor(pred_logi_full$def)

ggplot(pred_logi_full, aes(x = value, fill = key)) + 
...
true_and_predval <- cbind.data.frame(test$loan_st, pred_full_20,
                                     test$loan_st==pred_full_20)
# Show some values.
...
head(data.frame(true_and_predval[,1:2], decision = 
                ...
                evaluation = ifelse(true_and_predval$pred_full_20 == 1, 
                                    ...
                                    ifelse(true_and_predval$pred_full_20 == 0 &
  true_and_predval$`test$loan_st` == 0,
...
true_and_predval <- cbind.data.frame(test$loan_st, pred_full_35)
...
Let's apply the function to the <tt>`predictions_all_full`</tt> and the <tt>`logi_age`</tt>.
...
strategy_pred_log_model <- strategy_bank(as.numeric(pred_logi_age))
...
```{r}
ROC_all_full <- roc(test$loan_st, predictions_all_full)
ROC_log_model <- roc(test$loan_st, pred_logi_age)
